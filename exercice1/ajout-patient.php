    ...
    <body>
        ...
        <form method="POST" class="jumbotron center-block">
            <legend class="text-center h1_ajout_patient">Formulaire d'inscription</legend>
                ...
                <label for="lastName">Nom : </label><input type="text" name="lastname" id="lastName" required /><br/>
                <!-- Message d'erreur -->
                <span class="red"><?php if(!empty($_POST['lastname']) && !empty($_POST['lastname']) != $regexLastname){ echo $error_lastname; } ?></span><br/>
                
                <label for="firstName">Prénom : </label><input type="text" name="firstname" id="firstName" required /><br/>
                <span class="red"><?php if(!empty($_POST['firstname']) && !empty($_POST['firstname']) != $regexFirstname){ echo $error_firstname; } ?></span><br/>
                
                <label for="date">Date de naissance : </label><input type="date" name="birthdate" id="date" required /><br/><br/>
                
                <label for="phone">Téléphone : </label><input type="tel" name="phone" id="phone" required /><br/>
                <span class="red"><?php if(!empty($_POST['phone']) && !empty($_POST['phone']) != $regexPhone){ echo $error_phone; } ?></span><br/>
                
                <label for="email">Adresse e-mail : </label><input type="email" name="email" id="email" required /><br/>
